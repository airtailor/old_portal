  ...
  def index
    ...
    if @order.arrived == true && @order.counter == 0
      if @customer.phone.blank?
        ...
        SendSonar.message_customer(text: "Hi " + @customer.first_name.capitalize + ", just a heads up that your Air Tailor order (" + @order.shopify_id + ") has been received! We're going to get to work. When we're done, we'll ship it directly to " + @customer.address1 + ", " + @customer.address2 + ", " + @customer.city ", " + @customer.state + " " + @customer.zip + ". Is that still correct? Thank you!", to: @customer.phone)
      end
